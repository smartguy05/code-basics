//! Commands behind the installable **Roslyn / LSP** MCP server.
//!
//! Deliberately the same five-command shape as [`crate::commands::mcp`] and
//! [`crate::commands::browser_mcp`] — plan, confirm, apply, re-read the status
//! from disk — because it is the same machinery underneath:
//! [`cb_core::roslyn::install`] reuses `mcp::install`'s `config_target`,
...
//!
//! The three servers are installed under different names, carry different
...
//! sees behind a parameter; the Roslyn server grants read-only *code
//! intelligence* over one repository, which is a different thing to reason about
//! than a database login or a live browser session.

use tauri::State;

use cb_core::intents::providers::{self, InstallPlan, InstallScope};
use cb_core::intents::ProviderId;
use cb_core::roslyn::install as roslyn_install;

use crate::state::AppState;

/// Where the Roslyn MCP server is installed for this workspace and `provider`,
/// if anywhere. Project scope wins over user scope, as it does for the hooks.
#[tauri::command]
pub async fn roslyn_mcp_server_status(
    state: State<'_, AppState>,
    provider: ProviderId,
) -> Result<Option<InstallScope>, String> {
    let root = state.workspace_root()?;
    Ok(roslyn_install::status_for(provider, &root, None))
}

/// Exactly what installing the server for `provider` at `scope` would write.
/// Touches nothing.
#[tauri::command]
pub async fn roslyn_mcp_server_install_plan(
    state: State<'_, AppState>,
    provider: ProviderId,
    scope: InstallScope,
) -> Result<InstallPlan, String> {
    let root = state.workspace_root()?;
    roslyn_install::install_plan_for(provider, &root, scope, None).map_err(|e| format!("{e:#}"))
}

/// Perform an install the user has confirmed, returning the new status.
#[tauri::command]
pub async fn install_roslyn_mcp_server(
    state: State<'_, AppState>,
    provider: ProviderId,
    scope: InstallScope,
) -> Result<Option<InstallScope>, String> {
    let root = state.workspace_root()?;
    let plan = roslyn_install::install_plan_for(provider, &root, scope, None)
        .map_err(|e| format!("{e:#}"))?;
...
#[tauri::command]
pub async fn roslyn_mcp_server_uninstall_plan(
    state: State<'_, AppState>,
    provider: ProviderId,
    scope: InstallScope,
) -> Result<InstallPlan, String> {
    let root = state.workspace_root()?;
    roslyn_install::uninstall_plan_for(provider, &root, scope, None).map_err(|e| format!("{e:#}"))
}

/// Perform a removal the user has confirmed, returning the new status.
#[tauri::command]
pub async fn uninstall_roslyn_mcp_server(
    state: State<'_, AppState>,
    provider: ProviderId,
    scope: InstallScope,
) -> Result<Option<InstallScope>, String> {
    let root = state.workspace_root()?;
    let plan = roslyn_install::uninstall_plan_for(provider, &root, scope, None)
        .map_err(|e| format!("{e:#}"))?;
    providers::apply_writes_atomically(&plan.writes).map_err(|e| format!("{e:#}"))?;
    Ok(roslyn_install::status_for(provider, &root, None))
}
