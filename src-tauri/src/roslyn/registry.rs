//! Publishing what an `mcp-roslyn` client needs to find this application.
//!
//! The registry itself — its shape, its tolerance of a corrupt file, and the five
//! reasons there may be no instance to talk to — is
//! [`cb_core::roslyn::instances`]. This module is the application's side of it:
...
//!
//! Unlike the browser registry (whose listener comes and goes with a panel), the
//! Roslyn pipe is opened once at startup and lives for the process, so a running
//! application's entry always names a [`Listener`]. There is therefore no "off"
//! state to publish and no `Option` around the listener.
//!
...
//! changes the `workspaces` list [`cb_core::roslyn::instances::choose_instance`]
//! narrows on). Everything else about a session — whether it has finished loading,
//! what it would answer — is live state a client learns by connecting, and putting
//! it in the file would guarantee a stale answer.

use cb_core::roslyn::instances::{self, Listener, RoslynInstance, PROTOCOL_VERSION};

use crate::state::AppState;

/// A fresh per-launch token: 32 bytes of randomness as 64 hex characters.
///
/// Two v4 UUIDs, matching [`crate::browser::registry::mint_token`] — a CSPRNG
...
/// ([`cb_core::roslyn::instances::Listener::token`]); it grants nothing, because
/// the boundary here is the `--workspace` scope, not a token.
...
///
/// Empty rather than a guess: [`cb_core::roslyn::liveness`] matches nothing
...
/// their own entries in the same file and they are not this process's to discard.
pub fn publish(instance: RoslynInstance) -> Result<(), String> {
...
pub fn republish(state: &AppState) {
    let Some(listener) = state.roslyn_pipe_published() else {
        return;
    };
    let entry = instance_for(std::process::id(), own_exe(), listener, open_roots(state));
    if let Err(error) = publish(entry) {
        eprintln!("code-basics: the roslyn instance registry could not be written: {error}");
    }
}
...
/// [`cb_core::roslyn::instances::InstanceError::NoneRunning`]) but it is worth
