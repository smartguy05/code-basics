...
fn answer(tool: &str, workspace: Option<&str>) -> ToolAnswer {
    ...
    std::env::set_var(
        "CB_EDITOR_INSTANCES_PATH",
        r"C:\nowhere\no-such-editor-registry.json",
    );
...
fn a_known_tool_with_no_workspace_refuses_as_no_workspace() {
    let answer = answer(cb_core::editor_context::tools::GET_ACTIVE_FILE, None);
    assert!(!answer.ok);
...
fn a_blank_workspace_is_treated_as_none() {
    let answer = answer(cb_core::editor_context::tools::GET_SELECTION, Some("   "));
    assert!(!answer.ok);
    assert_eq!(answer.code.as_deref(), Some("noWorkspace"));
}
...
fn an_advertised_tool_with_a_workspace_reports_that_no_application_is_running() {
    let answer = answer(
        cb_core::editor_context::tools::GET_ACTIVE_FILE,
        Some(r"C:\repo"),
    );
    assert!(!answer.ok);
