//! The editor-context control pipe: the one way an agent's `mcp-editor` server
//! reaches this application's live editor state.
//!
//! Windows only, and the module is `#[cfg(windows)]` rather than internally
//! branched — every pure layer beneath it ([`cb_core::editor_context::wire`],
...
//! The `mcp-editor` server itself also builds everywhere and reports
//! *unsupported*.
//!
//! # The name carries the pid, and that is not decoration
//!
//! `\\.\pipe\code-basics.editor.<pid>`. A fixed name would be wrong three ways:
...
//! [`cb_core::editor_context::instances::InstanceError::Ambiguous`] — which
//! **refuses** rather than picking.
//!
//! # The listener is process-global, opened once at startup
//!
//! Like the Roslyn pipe (and unlike the browser pipe, tied to a panel), this one
//! lives for the whole process: the editor state is per-workspace but the pipe is
//! one per application, and every call resolves its own `--workspace` afresh
//! through [`AppState::editor_context_for`].
//!
//! # The feature is re-checked on every call
//!
//! Whether the `EditorContextMcp` feature is on is read from the features file
//! here, per call, and handed to [`agent::answer`]. The frontend stops pushing
//! state while the feature is off, but a context pushed before it was switched off
//! is still in `AppState`; serving that would leak state the user has said to stop
//! sharing, so the feature flag — not the mere presence of a context — decides.
//!
//! # What the DACL does, and what it does not
//!
//! Identical to the Roslyn and browser pipes', and for the same reason: a named
//! pipe created with a NULL security descriptor grants read access to *Everyone*
//! and to the anonymous account. [`user_only_descriptor`] builds
//! `D:P(A;;GA;;;<this user's SID>)`. That stops **other users**, and that is *all*
//! it stops — another program running as this user can open the pipe, and the
//! per-launch token in the registry reduces that to "another program that can read
//! this user's `%APPDATA%`". Here the `--workspace` scope baked into the install is
//! the actual boundary; there is no consent gate to grant, because the questions
//! are read-only over an already-open local repository.
//!
...
use cb_core::editor_context::instances::Listener;
use cb_core::editor_context::tools;
use cb_core::editor_context::wire::{self, RequestProblem};
...
pub fn pipe_name(pid: u32) -> String {
    format!(r"\\.\pipe\code-basics.editor.{pid}")
}
...
async fn accept_loop(
    ...
    while running.load(Ordering::SeqCst) {
        if let Err(error) = server.connect().await {
            eprintln!("code-basics editor pipe: {error}");
            return;
        }
...
async fn answer_line(line: ndjson::Line, app: &AppHandle, token: &str) -> Option<Value> {
    ...
    let call = match tools::parse_call(&request.tool, request.arguments.as_object()) {
        Ok(call) => call,
        Err(error) => {
            // A tool name this build does not have. (Arguments are ignored — the
            // editor tools take none.) Sent back as a refusal rather than a
            // protocol error: the shim is what speaks MCP, and it turns this into
            // the tool execution error the specification asks for.
    ...
    // A request that named no workspace reaches no session — the boundary is
    // missing entirely. This is the pipe-side guard; the shim refuses the same
    // way before it ever connects.
    if request.workspace.trim().is_empty() {
        return Some(wire::answer_value(
            &id,
            &cb_core::editor_context::answer::EditorRefusal::NoWorkspace.answer(),
        ));
    }

    // Read the feature flag fresh, and the pushed context for the request's own
    // `--workspace`. The `State` guard is dropped before the `agent::answer`
    // call — both reads return owned values. `agent::answer` turns feature-off
    // into `Disabled` and a missing context into `NoContext`; see its module docs
    // on why the feature is re-checked here rather than trusted to the frontend.
    let enabled = feature_enabled();
    let context = {
        let state = app.state::<AppState>();
        state.editor_context_for(Path::new(&request.workspace))
    };
    let answer = agent::answer(enabled, context, call);
    Some(wire::answer_value(&id, &answer))
}

/// Whether the editor-context feature is switched on, read fresh from the
/// user-global features file.
fn feature_enabled() -> bool {
    features::load(&features::features_path()).is_enabled(FeatureId::EditorContextMcp)
}
...
// ---------------------------------------------------------------------------
// Win32 — identical to the Roslyn and browser pipes'; a named pipe with no
// explicit DACL grants Everyone read access, so the descriptor is mandatory, not
// a nicety.
// ---------------------------------------------------------------------------
...
fn create_instance(name: &str, first: bool) -> Result<NamedPipeServer, String> {
    ...
    server.map_err(|error| {
        format!(
            "the editor control pipe could not be created at {name}: {error}. Agent access to the \
             editor state is unavailable; the application itself is unaffected."
        )
    })
}
...
fn user_only_descriptor() -> Result<LocalDescriptor, String> {
    ...
    if ok == 0 || descriptor.is_null() {
        return Err(format!(
            "the editor control pipe's access rules could not be built ({}), so no pipe was \
             created. Rather than fall back to Windows' default — which grants read access to \
             Everyone — agent access to the editor state is unavailable.",
