...
/// Drive `call_tool` with the registry pointed at a file that does not exist — the
/// situation a client is in whenever no application is running.
fn answer(tool: &str, workspace: Option<&str>) -> ToolAnswer {
    let previous = std::env::var_os("CB_BUILD_INSTANCES_PATH");
    std::env::set_var(
        "CB_BUILD_INSTANCES_PATH",
        r"C:\nowhere\no-such-build-registry.json",
    );
...
fn an_unknown_tool_is_refused_by_name_before_anything_else() {
    let answer = answer("compile_everything", Some(r"C:\repo"));
    assert!(!answer.ok);
...
/// An unscoped install reaches no build, so a known tool with no `--workspace` is
/// refused as `noWorkspace` — before any registry is read, and distinct from "no
/// application is running".
#[test]
#[cfg(windows)]
fn a_known_tool_with_no_workspace_refuses_as_no_workspace() {
    let answer = answer(cb_core::build::mcp::tools::GET_BUILD_STATUS, None);
    assert!(!answer.ok);
...
fn a_blank_workspace_is_treated_as_none() {
    let answer = answer(cb_core::build::mcp::tools::GET_ERRORS, Some("   "));
    assert!(!answer.ok);
    assert_eq!(answer.code.as_deref(), Some("noWorkspace"));
}

/// The name guard is only the *name*: an advertised tool with a workspace goes on to
/// look for an application, so a name guard cannot swallow the real refusal.
#[test]
#[cfg(windows)]
fn an_advertised_tool_with_a_workspace_reports_that_no_application_is_running() {
    let answer = answer(
        cb_core::build::mcp::tools::GET_BUILD_STATUS,
        Some(r"C:\repo"),
    );
    assert!(!answer.ok);
