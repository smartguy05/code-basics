//! The `mcp-editor` mode: this executable as an MCP stdio server that forwards
//! editor-state questions to a *running* application over a named pipe.
//!
//! The seventh thing this binary can be, after the application, the intent
//! recorder, the quality gate, the SQL MCP server, the browser MCP server, the
//! Tasks MCP server and the Roslyn MCP server — and like those it re-invokes
//! itself rather than shipping a second artifact
//! ([`cb_core::editor_context::argv`]).
//!
//! **Everything it decides lives in [`cb_core::editor_context`]**: which
//! application to talk to and the five reasons there may be none (`instances`),
//! what a request and a reply look like (`wire`), the four tools and their schemas
//! (`tools`), the handshake (`serve`). What is left here is I/O — read stdin,
//! connect a pipe, write a line, wait — and it is here precisely because none of
//! it is reachable from a test.
//!
//! # It is the Roslyn server's twin
//!
//! Like `mcp_roslyn` (and `mcp_browser`), this server answers nothing itself: the
//! state lives in a window in another process, so every call is forwarded and the
//! reply is the application's own words. Like the Roslyn server — and unlike the
//! browser server — the boundary is the `--workspace` the entry was installed
//! with, so an agent configured for repo X reaches repo X regardless of which
//! window is focused.
//!
...
//!
//! # A connection per call, and a modest deadline
//!
...
//! a stale handle. [`TOOL_TIMEOUT`] is short compared with the Roslyn server's:
//! reading the live editor state is a map lookup, not a solution-wide analysis, so
//! anything slow here is a stuck application rather than legitimate work, and a
//! timeout is its own answer
//! ([`cb_core::editor_context::wire::PipeFailure::Timeout`]), never folded into
//! "not running".

use std::time::Duration;

use cb_core::editor_context::answer::EditorRefusal;
use cb_core::editor_context::instances::{self, InstanceError};
use cb_core::editor_context::wire::{self, PipeFailure, ToolAnswer};
use cb_core::editor_context::{argv, liveness, serve, tools};
...
///
/// 15 s — a quarter of the Roslyn server's. Reading the pushed editor state is a
/// map lookup in the application; anything that takes this long is a stuck
/// application, not legitimate work, and saying so beats waiting a minute.
...
/// application?
pub fn is_mcp_editor_invocation() -> bool {
    argv::is_mcp_editor_invocation(&std::env::args().collect::<Vec<_>>())
}

/// Serve until stdin closes, then exit.
pub fn run() -> ! {
    let args: Vec<String> = std::env::args().collect();
    let Some(invocation) = argv::parse_mcp_editor_args(&args) else {
...
async fn answer_request(
    ...
    match route {
        ...
        mcp_serve::Route::ToolsCall { name, arguments } => {
            // The arguments are **not** parsed here — the editor tools take none,
            // and the application ignores them anyway. The name is forwarded as
            // sent.
...
async fn call_tool(
    ...
    if !load_gate().is_enabled(tool_gate::ServerId::EditorContext, tool) {
    ...
    let Some(workspace) = workspace.map(str::trim).filter(|w| !w.is_empty()) else {
        return EditorRefusal::NoWorkspace.answer();
    };
...
fn unsupported() -> ToolAnswer {
    ToolAnswer::refused(
        "unsupported",
        format!(
            "The editor-context MCP server reaches the running application over a Windows named \
             pipe; {} has no such facility, so there is nothing to forward to.",
            std::env::consts::OS
        ),
    )
}
