//! The `mcp-build` mode: this executable as an MCP stdio server that forwards
//! build questions to a *running* application over a named pipe.
//!
//! The eighth thing this binary can be, after the application, the intent recorder,
//! the quality gate, the SQL MCP server, the browser MCP server, the Tasks MCP
//! server, the Roslyn MCP server and the editor-context MCP server — and like those
//! it re-invokes itself rather than shipping a second artifact
//! ([`cb_core::build::mcp::argv`]).
//!
//! **Everything it decides lives in [`cb_core::build::mcp`]**: which application to
//! talk to and the five reasons there may be none (`instances`), what a request and
//! a reply look like (`wire`), the four tools and their schemas (`tools`), the
//! handshake (`serve`). What is left here is I/O — read stdin, connect a pipe, write
//! a line, wait — and it is here precisely because none of it is reachable from a
//! test.
//!
//! # It is the Roslyn server's twin, not the SQL server's
//!
//! Like `mcp_roslyn` (and `mcp_editor`), this server answers nothing itself: the
//! build runs in a window in another process, so every call is forwarded and the
//! reply is the application's own words. Like the Roslyn server the boundary is the
//! `--workspace` the entry was installed with, so an agent configured for repo X
//! builds repo X regardless of which window is focused.
//!
...
//!
//! # A connection per call, and a very generous deadline
//!
//! The registry is re-read per call, so an application started after this server did
//! is found and one that exited is reported as gone rather than answered from a
//! stale handle. [`TOOL_TIMEOUT`] is the longest of any server's: a real build of a
//! large solution legitimately takes minutes, and a timeout is its own answer
//! ([`cb_core::build::mcp::wire::PipeFailure::Timeout`]), never folded into "not
//! running".

use std::time::Duration;

use cb_core::build::mcp::answer::BuildRefusal;
use cb_core::build::mcp::instances::{self, BuildInstance, InstanceError};
use cb_core::build::mcp::wire::{self, PipeFailure, ToolAnswer};
use cb_core::build::mcp::{argv, serve, tools};
...
///
/// 20 minutes — by far the longest of any server's, and deliberately so:
/// `build_solution` runs a real `dotnet build`, which on a large solution takes
/// minutes of legitimate work. Saying "still building" after twenty minutes beats
/// telling an agent the build failed when it did not.
const TOOL_TIMEOUT: Duration = Duration::from_secs(20 * 60);
...
///
/// The build MCP module has no `liveness` module of its own; the probe and identity
/// comparison live once in [`cb_core::mcp::liveness`], shared with every registry.
fn alive(instance: &BuildInstance) -> bool {
    cb_core::mcp::liveness::alive_by(instance.pid, &instance.exe)
}

/// Did the command line ask for the Build MCP server rather than the application?
pub fn is_mcp_build_invocation() -> bool {
    argv::is_mcp_build_invocation(&std::env::args().collect::<Vec<_>>())
}

/// Serve until stdin closes, then exit.
pub fn run() -> ! {
    let args: Vec<String> = std::env::args().collect();
    let Some(invocation) = argv::parse_mcp_build_args(&args) else {
...
async fn answer_request(
    ...
    match route {
        ...
        mcp_serve::Route::ToolsCall { name, arguments } => {
            ...
            // because it is the side that owns the tool table. The name is forwarded
            // as sent.
...
async fn call_tool(
    ...
    // The name is checked here, before any registry is read, so an unknown tool is
    // not answered by "start code-basics and retry" about a tool that will never
    // exist. Only the name: the application still owns the tool table.
    if !tools::is_known(tool) {
        return serve::unknown_tool_answer(tool);
    }

    // A disabled-but-known tool is refused before a registry is read, re-reading the
    // gate every call. See `cb_core::tool_gate`.
    if !load_gate().is_enabled(tool_gate::ServerId::Build, tool) {
        return serve::disabled_tool_answer(tool);
    }

    // The `--workspace` scope *is* the boundary. An unscoped install reaches no
    // build, so it is refused here — before a registry is read — rather than
    // guessing a directory.
    let Some(workspace) = workspace.map(str::trim).filter(|w| !w.is_empty()) else {
        return ToolAnswer::refused(
            BuildRefusal::NoWorkspace.code(),
            BuildRefusal::NoWorkspace.sentence(),
        );
    };

    let path = instances::instances_path();
    let file = instances::load(&path);
    let instance = match instances::choose_instance(&file, workspace, hint, &alive) {
    ...
    // The pipe a client opens is derived from the verified pid, never the string the
    // registry stated — `choose_instance` already refused a stated name that
...
/// everywhere the way the Roslyn server does.
fn unsupported() -> ToolAnswer {
    ToolAnswer::refused(
        "unsupported",
        format!(
            "The Build MCP server reaches the running application over a Windows named pipe; {} has \
             no such facility, so there is nothing to forward to.",
            std::env::consts::OS
        ),
    )
}
