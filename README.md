...
- **Run** — discovers runnable projects, `launchSettings.json` profiles, and `package.json` scripts; launches them with streamed output in per-run console tabs, and kills the whole process tree on cancel. Configurations live in a titlebar dropdown (status dots, favourites, custom ordering); an `ASPNETCORE_ENVIRONMENT` picker, build/rebuild/clean actions, and a **Secrets…** button that opens a .NET project's user secrets as an ordinary editor tab round out the tab.
...
- **Notes** — a floating, resizable scratchpad for free-form notes and prompts-for-later, with several named notes in one panel (add, rename, delete) and minimize to a thin labeled bar. Notes are stored globally (in your user config directory, not per-workspace), so the same scratchpad is available in every project. Any note can be **sent to an agent** (run as a prompt in the agent panel) or **saved as an instruction** so it joins the Enhancements → Add Instructions library.
