...
- **Multiple codebases at once** — open several projects as tabs above the view row and switch between them instantly. Each keeps its own runs, tests, terminals and language server alive in the background, so switching interrupts nothing; the active codebase's folder, path and branch show in the app chrome, and a background codebase whose terminal wants attention flashes its tab.
