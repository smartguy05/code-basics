//! Is this process the Build MCP server, or the application?
//!
//! A self-dispatch mode beside [`crate::intents::hook`]'s `record-intent`,
//! [`crate::qgate`]'s `quality-gate`, [`crate::mcp::argv`]'s `mcp-sql`, the browser
//! server's `mcp-browser`, the Tasks server's `mcp-tasks`, the Roslyn server's
//! `mcp-roslyn` and the editor-context server's `mcp-editor`, and it is here for
//! the same three reasons: the installed MCP entry runs *this executable*, so there
//! is no second artifact to keep in step, no interpreter to depend on, and no
//! version skew between the server and the app it was installed from.
//!
...
//! (`.mcp.json`, `~/.claude.json`, `~/.codex/config.toml`): a hand-edited line that
//! kept only the subcommand, or only the marker, must still start the server. The
//! marker is read back through the same constant that writes it.
//!
//! # `--workspace` *is* the boundary
//!
//! `--workspace <root>` names the one repository this server may reach — the Tasks
//! pattern. An unscoped invocation reaches no session and every tool answers
//! [`super::answer::BuildRefusal::NoWorkspace`] rather than guessing a directory.
//! The install step bakes it in, so it is the consent boundary.
//!
//! # `--instance` is a disambiguator, not a permission
//!
//! It names a process id when several windows have the same repository open, and
//! that is **all** it does. See [`super::instances::choose_instance`].

/// The subcommand the installed MCP entry runs.
pub const SUBCOMMAND: &str = "mcp-build";
...
pub const MARKER: &str = "code-basics-mcp-build";
...
pub fn is_mcp_build_invocation(args: &[String]) -> bool {
    let marker = format!("--{MARKER}");
    args.iter().any(|arg| arg == SUBCOMMAND || *arg == marker)
}

/// What an `mcp-build` command line asked for.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct McpBuildInvocation {
...
/// Read an `mcp-build` command line. [`None`] means this is not a Build MCP
/// invocation and there is nothing to do — an ordinary application launch.
pub fn parse_mcp_build_args(args: &[String]) -> Option<McpBuildInvocation> {
    if !is_mcp_build_invocation(args) {
        return None;
    }
    Some(McpBuildInvocation {
