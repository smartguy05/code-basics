//! Which running application an editor-context MCP client should talk to — and
//! the five different reasons there might not be one.
//!
//! The **twin** of [`crate::roslyn::instances`]. The editor MCP server is a
//! **separate process** (`cb-app mcp-editor`, self-dispatched) started by an
//! agent, not by the user. It has no window, no `AppState` and no way to know
//! whether an application is running at all, so the application publishes what a
//! client needs into a small user-global registry and this module is everything
//! that is decided about it.
//!
//! # Narrowing on the workspace, not on a panel
//!
//! This server's boundary is `--workspace <root>`: [`choose_instance`] narrows on
//! **which running application has that workspace open** ([`EditorInstance::workspaces`]).
...
//!
//! The editor pipe is opened once at application startup and every call resolves
//! the workspace afresh, so a live application always carries a [`Listener`].
//! There is therefore no "panel closed" state here: the five refusals are
...
const INSTANCES_FILE: &str = "editor-instances.json";

/// Where an application publishes its pipe, and how a client presents itself.
///
/// Present whenever the application is running: the editor pipe is process-global
/// (opened at startup), so unlike the browser's, this is not an `Option`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Listener {
    /// The full pipe name, `\\.\pipe\code-basics.editor.<pid>`.
    ///
    ...
    /// as the application, fabricating editor state straight into an agent's
    /// transcript. A value here that disagrees with the derived name is *evidence
    /// of tampering* — see [`InstanceError::PipeNameMismatch`].
    pub pipe: String,
...
/// already verified against [`EditorInstance::exe`]; it must never open the name
...
pub struct InstancesFile {
    pub version: u32,
    #[serde(default)]
    pub instances: Vec<EditorInstance>,
}
...
impl InstanceError {
    ...
    pub fn code(&self) -> &'static str {
        match self {
            ...
            Self::PipeNameMismatch { .. } => "editor_registry_tampered",
        }
    }
    ...
    pub fn sentence(&self) -> String {
        match self {
            Self::NoneRunning { hint: None } => "No code-basics application is running, so there \
                 is no editor to ask. Nothing was read. Start code-basics and open the repository \
                 this server is scoped to."
                .to_string(),
            ...
            Self::WorkspaceNotOpen { workspace, running } => {
                ...
                format!(
                    "code-basics is running (pids {list}), but no window has the workspace \
                     {workspace:?} open, so its editor is not available. Nothing was read. Open \
            ...
            Self::VersionMismatch { pid, protocol } => format!(
                "code-basics (pid {pid}) speaks editor protocol {protocol} and this MCP server \
                 speaks {PROTOCOL_VERSION}, so nothing was sent. The running application and the \
                 executable in the MCP configuration are different builds. Reinstall the editor \
            ...
            Self::PipeNameMismatch { pid, stated } => format!(
                "The code-basics instance registry names the control pipe \"{stated}\" for pid \
                 ...
                 to another program answering as the application. Close code-basics, delete \
                 editor-instances.json from your user config directory, and start it again."
            ),
        }
    }
}
...
///
/// The same base resolution as [`crate::roslyn::instances::instances_path`], with
/// `CB_EDITOR_INSTANCES_PATH` overriding the whole path — the escape hatch that
/// makes the pipe and the shim drivable by hand.
pub fn instances_path() -> PathBuf {
    if let Some(path) = std::env::var_os("CB_EDITOR_INSTANCES_PATH") {
...
/// window.
pub fn upsert(file: &mut InstancesFile, instance: EditorInstance) {
...
pub fn choose_instance<'a>(
    ...
    workspace: &str,
    hint: Option<&str>,
    alive: &dyn Fn(&EditorInstance) -> bool,
) -> Result<&'a EditorInstance, InstanceError> {
    let hint = hint.map(str::trim).filter(|h| !h.is_empty());

    let named: Vec<&EditorInstance> = match hint {
        Some(wanted) => file
            .instances
            .iter()
    ...
    let live: Vec<&EditorInstance> = named.into_iter().filter(|entry| alive(entry)).collect();
    if live.is_empty() {
        return Err(none_running());
    }

    let speakable: Vec<&EditorInstance> = live
        .iter()
        .copied()
    ...
    let matching: Vec<&EditorInstance> = speakable
        .iter()
        .copied()
