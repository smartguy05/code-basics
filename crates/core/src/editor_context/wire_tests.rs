...
use crate::editor_context::instances::PROTOCOL_VERSION;
...
fn request() -> Request {
    Request {
        ...
        tool: "get_active_file".to_string(),
...
fn another_method_on_this_pipe_is_refused_by_name() {
    let problem = parse_request("editor.other", Some(&params_of(&request())), TOKEN).unwrap_err();
    assert_eq!(
        problem,
        RequestProblem::UnknownMethod {
            method: "editor.other".to_string()
        }
    );
    assert!(problem.sentence().contains("editor.other"));
}
...
fn the_token_refusal_does_not_leak_the_expected_token() {
    ...
    assert!(!sentence.contains(TOKEN), "{sentence}");
}
...
fn an_answer_round_trips_with_the_id_it_was_given() {
    let encoded = answer_value(&json!(3), &ToolAnswer::ok("active file: src/App.cs"));
    assert_eq!(encoded["id"], 3);
    let parsed = parse_answer(&encoded).unwrap();
    assert!(parsed.ok);
    assert_eq!(parsed.code, None);
    assert_eq!(parsed.text, "active file: src/App.cs");
}
...
fn a_refusal_carries_its_code_and_survives_the_round_trip() {
    let refusal = ToolAnswer::refused("noActiveFile", "no file is active");
...
fn a_timeout_is_not_reported_as_a_closed_application() {
    ...
    assert!(sentence.contains("15000"), "{sentence}");
    assert!(sentence.contains("get_active_file"), "{sentence}");
