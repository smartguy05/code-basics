...
use crate::editor_context::instances::{EditorInstance, Listener, PROTOCOL_VERSION};
use std::path::PathBuf;

fn entry(pid: u32, exe: &str) -> EditorInstance {
    EditorInstance {
        pid,
        exe: exe.to_string(),
        protocol: PROTOCOL_VERSION,
        listener: Listener {
            pipe: crate::editor_context::instances::pipe_name(pid),
            token: "t".to_string(),
        },
