//! The editor-context MCP server's handshake, and the shape of every answer it
//! sends.
//!
//! The **twin** of [`crate::roslyn::serve`]. The runner in the app crate
//! (`src-tauri/src/mcp_editor.rs`) is I/O and nothing else, for the reason
//! [`crate::mcp::serve`] gives about its own runner: a binary that reads stdin and
//! opens a named pipe is not reachable from a test. So everything between "a line
//! arrived" and "a line goes back" that is a *choice* lives here or in
//! [`super::tools`], [`super::wire`] and [`super::instances`].
//!
...
//! so this server reuses them unchanged — the same decision the browser, Tasks
//! and Roslyn servers made.
//!
...
pub const SERVER_NAME: &str = "code-basics-editor";
...
///
/// Rules a model cannot infer and will otherwise get wrong: this is the *live*
/// state of the human's editor rather than a search of the files, it can abstain
/// honestly (no file active, nothing selected, the feature off), it takes no
/// arguments, and the answer is scoped to the one workspace this server was
/// installed for.
pub const INSTRUCTIONS: &str = "These tools report what the user is looking at right now in the \
code-basics editor for one workspace: the active file, the cursor and viewport, the current text \
selection, the open tabs, and the recently edited files. This is the user's live editor state, not \
a search of the repository — it is what is on their screen this moment.

Three things to know. The tools take no arguments: each reports the current state directly, and you \
cannot point them at a file or position. Every tool can abstain honestly rather than fabricate: \
\"no file is active\", \"nothing is selected\" and \"the feature is switched off\" are real replies \
with their own codes, and none of them invents a path or a cursor. Positions are line (1-based) and \
character (0-based UTF-16 code units). And the answers are scoped to the workspace this server was \
installed for; it cannot see another repository.";
...
pub fn tools_list_result(gate: &crate::tool_gate::ToolGateFile) -> Value {
    json!({
        "tools": crate::tool_gate::filter_descriptors(
            gate,
            crate::tool_gate::ServerId::EditorContext,
            tools::descriptors(),
        )
    })
}
...
/// name a window, switch the feature on, open a file.
