//! The one platform seam of the Roslyn instance registry: is this entry's
//! process still the application that wrote it?
//!
//! Thin on purpose, exactly like [`crate::browser::liveness`] — it makes one
//! query and shapes the answer — so that [`super::instances::choose_instance`]
//! can stay pure and take liveness as a parameter.
//!
...
//! process nobody identified for a semantic answer.

use super::instances::RoslynInstance;

// The probe and the identity comparison live once in [`crate::mcp::liveness`],
// shared with the browser registry; this re-export keeps `same_executable`
...
pub fn alive(instance: &RoslynInstance) -> bool {
