...
fn the_server_names_itself_apart_from_the_other_servers() {
    assert_eq!(SERVER_NAME, "code-basics-roslyn");
...
fn the_instructions_state_the_rules_a_model_cannot_infer() {
    assert!(INSTRUCTIONS.contains("not a text search"), "{INSTRUCTIONS}");
    assert!(INSTRUCTIONS.contains("abstain"), "{INSTRUCTIONS}");
    assert!(INSTRUCTIONS.contains("position") && INSTRUCTIONS.contains("symbol"));
    assert!(
...
fn a_successful_answer_is_a_successful_result() {
    let result = answer_result(&ToolAnswer::ok("3 usages"));
    assert_ne!(result.is_error, Some(true));
    let rendered = serde_json::to_value(&result).unwrap();
    assert!(rendered.to_string().contains("3 usages"));
}

#[test]
fn a_refusal_is_a_tool_execution_error_and_carries_its_code() {
    let result = answer_result(&ToolAnswer::refused("serverLoading", "still loading"));
    assert_eq!(result.is_error, Some(true));
    assert_eq!(
        result.structured_content,
        Some(serde_json::json!({ "code": "serverLoading" }))
    );
    let text = serde_json::to_value(&result).unwrap().to_string();
    assert!(text.contains("serverLoading"), "{text}");
...
fn the_shim_unknown_tool_wording_matches_the_application_parse() {
    assert_eq!(
        ...
        crate::roslyn::tools::unknown_tool_sentence("x")
    );
}
