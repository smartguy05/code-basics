...
fn the_entry_is_a_command_field_and_an_args_array() {
    ...
    assert_eq!(
        entry["args"],
        json!(["mcp-roslyn", "--workspace", r"C:\code\repo"])
    );
}
...
fn the_exe_path_is_never_quoted_into_a_command_string() {
    ...
    let entry = roslyn_mcp_entry(hostile, None);
...
fn the_workspace_is_a_separate_arg_and_never_quoted_into_one() {
    ...
    let entry = roslyn_mcp_entry("cb-app.exe", Some(Path::new(r"C:\my repo\proj")));
    ...
    let parsed = argv::parse_mcp_roslyn_args(&command_line).unwrap();
    assert_eq!(parsed.workspace.as_deref(), Some(r"C:\my repo\proj"));
}

#[test]
fn a_user_scope_entry_writes_no_workspace() {
    let entry = roslyn_mcp_entry("cb-app.exe", None);
    assert_eq!(entry["args"], json!(["mcp-roslyn"]));
}
...
fn a_project_install_bakes_in_the_workspace() {
    ...
    let args = parsed["mcpServers"]["code-basics-roslyn"]["args"]
        .as_array()
        .unwrap();
    assert_eq!(args[0], "mcp-roslyn");
...
fn a_codex_install_writes_config_toml_and_a_project_scope_is_refused() {
    ...
    assert!(plan.writes[0]
        .content
        .contains("[mcp_servers.code-basics-roslyn]"));
...
fn installing_beside_the_other_servers_keeps_them_all() {
    ...
    .unwrap();
    assert!(
        roslyn.writes[0].merges_existing,
        "an existing config must be merged into, and backed up"
    );
    let parsed: Value = serde_json::from_str(&roslyn.writes[0].content).unwrap();
    let servers = parsed["mcpServers"].as_object().unwrap();
    assert!(servers.contains_key("code-basics-sql"), "{servers:?}");
    assert!(servers.contains_key("code-basics-roslyn"), "{servers:?}");
}
...
fn uninstalling_removes_only_our_entry() {
    ...
    std::fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            "mcpServers": {
                "code-basics-roslyn": { "command": "x", "args": ["mcp-roslyn"] },
    ...
    let parsed: Value = serde_json::from_str(&plan.writes[0].content).unwrap();
    let servers = parsed["mcpServers"].as_object().unwrap();
    assert!(!servers.contains_key("code-basics-roslyn"));
...
#[test]
fn the_access_note_says_read_only_and_names_no_write_tool() {
