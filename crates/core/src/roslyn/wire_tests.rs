...
use crate::roslyn::instances::PROTOCOL_VERSION;
...
fn another_method_on_this_pipe_is_refused_by_name() {
    let problem = parse_request("roslyn.other", Some(&params_of(&request())), TOKEN).unwrap_err();
    assert_eq!(
        problem,
        RequestProblem::UnknownMethod {
            method: "roslyn.other".to_string()
        }
    );
    assert!(problem.sentence().contains("roslyn.other"));
}
...
fn an_answer_round_trips_with_the_id_it_was_given() {
    let encoded = answer_value(&json!(3), &ToolAnswer::ok("3 usages"));
    assert_eq!(encoded["id"], 3);
    let parsed = parse_answer(&encoded).unwrap();
    assert!(parsed.ok);
    assert_eq!(parsed.code, None);
    assert_eq!(parsed.text, "3 usages");
}
...
fn a_refusal_carries_its_code_and_survives_the_round_trip() {
    let refusal = ToolAnswer::refused("noSession", "no session for that workspace");
...
fn the_four_pipe_failures_have_four_codes_and_four_sentences() {
    let failures = [
        ...
        PipeFailure::Closed {
            tool: "get_diagnostics".to_string(),
        },
...
fn a_timeout_is_not_reported_as_a_closed_application() {
    ...
    assert!(sentence.contains("find_references"), "{sentence}");
