...
/// shape.
fn open(pid: u32) -> RoslynInstance {
    RoslynInstance {
        pid,
...
fn other_repo(pid: u32) -> RoslynInstance {
    RoslynInstance {
...
fn file(instances: Vec<RoslynInstance>) -> InstancesFile {
    InstancesFile {
        version: 1,
        instances,
    }
...
fn all_alive(_: &RoslynInstance) -> bool {
    true
}

fn none_alive(_: &RoslynInstance) -> bool {
    false
}
...
fn the_liveness_probe_sees_the_whole_entry_so_a_recycled_pid_can_be_refused() {
    ...
    let probe = |instance: &RoslynInstance| {
...
fn a_dead_window_does_not_make_a_live_one_ambiguous() {
    ...
    let only_two = |instance: &RoslynInstance| instance.pid == 2;
    assert_eq!(
...
fn the_pipe_name_is_a_function_of_the_pid() {
    assert_eq!(r"\\.\pipe\code-basics.roslyn.1234", pipe_name(1234));
...
fn a_corrupt_file_loads_as_no_instances_rather_than_stopping_mcp_starting() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("roslyn-instances.json");
...
fn a_saved_file_reads_back() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("nested").join("roslyn-instances.json");
...
fn saving_leaves_no_temp_file_behind() {
    ...
    let strays: Vec<String> = std::fs::read_dir(dir.path())
        ...
        .filter(|name| name != "roslyn-instances.json")
        .collect();
...
fn the_path_is_overridable_the_way_every_other_user_global_store_is() {
    ...
    std::env::set_var("CB_ROSLYN_INSTANCES_PATH", r"C:\tmp\somewhere.json");
    assert_eq!(
        instances_path(),
