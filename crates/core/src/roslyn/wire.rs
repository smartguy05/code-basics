//! What travels on the Roslyn control pipe, in both directions.
//!
//! Two processes: the **shim** (`cb-app mcp-roslyn`, started by an agent, no
//! window, speaks MCP on its stdio) and the **application** (the one holding the
//! warm `LspHandle`). The shim owns no semantic model and decides nothing about
//! the code; the application owns all of it. So the pipe carries a tool call one
//! way and *the words the agent will read* the other way — [`super::render`]
//! composes the prose inside the application, and the shim's whole job is
//! wrapping a [`ToolAnswer`] in a `CallToolResult`.
//!
//! # Framing is not here
//!
...
//! [`super::tools::RoslynToolCall`] — an exhaustive enum — and a second dispatch
...
//!
//! Unlike the browser (whose request names no repository, because the panel
//! belongs to a window), [`Request::workspace`] carries the `--workspace`
//! boundary the shim was installed with. The shim uses it to choose *which*
//! application to connect to ([`super::instances::choose_instance`]); the
//! application uses it to resolve *which* `LspHandle` answers. Both need it, so
//! it is on the wire rather than derived twice.

use serde::{Deserialize, Serialize};
use serde_json::{json, Value};

/// The only method on this pipe.
pub const METHOD: &str = "roslyn.tool";
...
pub struct Request {
    ...
    /// application resolves it to the `LspHandle` that answers.
    pub workspace: String,
    /// The MCP tool name, e.g. `find_references`.
    pub tool: String,
...
/// the workspace, name a window, wait for the server to finish loading.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
    ...
    pub fn sentence(&self) -> String {
        match self {
            ...
            Self::ProtocolMismatch { theirs, ours } => format!(
                "The caller speaks roslyn protocol {theirs} and this application speaks {ours}, so \
                 nothing was done. Reinstall the Roslyn MCP entry from this application."
            ),
            ...
            Self::BadToken => {
                "The token presented does not match this application's. Read it from \
                 the roslyn instance registry this application published."
                    .to_string()
            }
...
/// Read a `roslyn.tool` request, checking the method, the shape, the protocol
...
pub enum PipeFailure {
    ...
    /// folded into "not running": Roslyn `references` on a large solution is
    /// legitimately slow, and a timeout is a fact worth reporting rather than
    /// hiding.
        ...
        match self {
            ...
            Self::Timeout { tool, ms } => format!(
                "{tool} did not answer within {ms}ms, so nothing can be reported. This is not the \
                 ...
                 which for a large solution can mean the language server is still working."
            ),
            ...
            Self::Malformed { detail } => format!(
                "The application's reply could not be read ({detail}), so its answer is unknown. \
                 ...
                 the Roslyn MCP entry from the running application."
            ),
        }
    }
