...
fn the_subcommand_alone_starts_the_server() {
    assert!(is_mcp_roslyn_invocation(&args(&[
        r"C:\apps\cb-app.exe",
        "mcp-roslyn"
    ])));
}
...
fn an_ordinary_launch_is_not_an_mcp_invocation() {
    ...
    assert!(parse_mcp_roslyn_args(&args(&[r"C:\apps\cb-app.exe", "."])).is_none());
}

#[test]
fn the_other_self_dispatch_modes_are_not_this_server() {
    // The six modes must not answer for each other: another mode reaching this
    // parser would start a Roslyn server on that client's stdio and hang it.
    for other in [
        "mcp-sql",
        "--code-basics-mcp-sql",
        "mcp-browser",
        ...
        assert!(
            !is_mcp_roslyn_invocation(&args(&[r"C:\apps\cb-app.exe", other])),
            "{other} must not start the roslyn server"
        );
    }
}
...
fn the_workspace_flag_is_the_boundary_and_is_read() {
    let parsed = parse_mcp_roslyn_args(&args(&[
        "cb-app.exe",
        "mcp-roslyn",
        "--workspace",
        r"C:\code\repo",
...
fn no_workspace_flag_is_an_unscoped_install() {
    ...
    let parsed = parse_mcp_roslyn_args(&args(&["cb-app.exe", "mcp-roslyn"])).unwrap();
...
fn an_empty_workspace_value_is_no_workspace() {
    for empty in ["", "   "] {
        let parsed =
            parse_mcp_roslyn_args(&args(&["cb-app.exe", "mcp-roslyn", "--workspace", empty]))
                .unwrap();
...
fn an_empty_instance_value_is_no_instance() {
    for empty in ["", "   "] {
        let parsed =
            parse_mcp_roslyn_args(&args(&["cb-app.exe", "mcp-roslyn", "--instance", empty]))
                .unwrap();
...
fn a_trailing_flag_with_no_value_is_none() {
    ...
    let parsed =
        parse_mcp_roslyn_args(&args(&["cb-app.exe", "mcp-roslyn", "--workspace"])).unwrap();
    assert_eq!(parsed.workspace, None);
    let parsed = parse_mcp_roslyn_args(&args(&["cb-app.exe", "mcp-roslyn", "--instance"])).unwrap();
    assert_eq!(parsed.instance, None);
}
