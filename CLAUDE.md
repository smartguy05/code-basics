...
- `pnpm debuggers:fetch` — vendors the NetCoreDbg and js-debug **debug adapters** into `src-tauri/resources/debuggers/` (pinned version + SHA-256, idempotent via a `.stamp`, soft-fails offline). Unlike the sidecar this **is** chained into `beforeBuildCommand`, so `pnpm tauri build` runs it for you; a checksum mismatch is deliberately fatal
...
**A local `Stop` hook enforces part of this gate automatically.** `.claude/settings.json` registers `cb-app.exe quality-gate` — a self-invoking subcommand, exactly like `record-intent`, not a shipped script. It runs when an agent turn ends and, deterministically (no model reasoning involved): runs `pnpm typecheck` if any `*.ts`/`*.tsx` changed and `cargo fmt --check` if any `*.rs` changed, **blocking the turn** (exit 2, feeding the failure back) until they pass; blocks on any changed file still carrying an unresolved `AI-REJECTED YYYY-MM-DD` note (the same thing the git `pre-commit` hook refuses, surfaced earlier); and prints a **non-blocking** reminder when a turn edited source but touched no `.memories/` file. **A failed check is not automatically a failed turn.** `qgate::read_gate_output` reads each non-zero run as one of *three* answers, because a gate can fail for reasons that have nothing to do with the change set — an unreadable pnpm junction, a running executable the linker cannot replace — and blocking on those wedges a turn that no edit could unblock. A run whose output describes the *machine* is `Unrunnable`: it never blocks, and it never passes silently either, printing to stderr that nothing was checked. Two rules keep that honest, and both are pinned by tests: it **never sifts a broken run** for the "real" errors among the cascade (once the dependency tree is unreachable the `any`-poisoned diagnostics are indistinguishable from genuine ones, and choosing would be the guess this codebase refuses), and a missing package is environmental **only if `package.json` already declares it** — importing something never installed stays an ordinary failure, as does a missing relative path. Heavier Rust checks (`cargo clippy`) are opt-in via `CB_GATE_FULL=1` — off by default because they relink and can hit the "app is running ⇒ Access denied" lock. All decisions live in `crates/core/src/qgate/` and are unit-tested there (`cargo test -p cb-core qgate::`); the thin runner is `src-tauri/src/qgate_run.rs`. It is **installable from the app** like the intent hooks (Changes → Intent → *Set up agent intent capture* → **Quality gate**), so it depends on a built `target/release/cb-app.exe` — rebuild release after changing `qgate/`. The hook complements, and does not replace, running the full commands yourself before claiming done.
...
1. **`crates/core` (`cb-core`)** — all decision-making logic, deliberately with **no Tauri dependency** so everything is unit-testable headlessly:
   ...
   - `adapters/` — per-ecosystem knowledge (`dotnet.rs`, `node.rs`, `cargo.rs`): how to detect a project and build the command line to run/test/build it. `manifest.rs` adds declarative TOML adapters loaded from `.code-basics/adapters/*.toml` in a workspace (see `examples/adapters/` for pytest and cargo-nextest); any runner that emits JUnit XML can be added without Rust code. .NET: no launch profile means `dotnet run`'s default profile applies (`ignore_launch_settings` opts out); detected test configs are Debug-only. **Detection emits one run configuration per (project, framework), not one per build configuration** — Debug/Release is a property of a launch, so it is a picker in the Run toolbar (`start_run`/`build_project` take a per-run `build_configuration` override) rather than three near-identical entries in the list. The surviving entry keeps its `:run:debug` id so a saved favourite still matches; the ids that vanished need no migration because `config::sort_configs` already ignores an id naming nothing, and a migration would have been *dangerous* — a launch profile's id is also `:run:{sanitised name}`. **Cargo is detection-only**: `cargo.rs` reads a `Cargo.toml` and emits **no** run or test configurations at all, because doing so would change the Run and Tests tabs for every Rust repository on the next scan. It exists so the architecture graph has nodes with `ecosystem == "cargo"`; `examples/adapters/cargo-nextest.toml` remains the way to actually run Rust, and the manifest-adapter merge in `workspace.rs` deliberately lets a declarative adapter supply configurations for a directory cargo already claimed (any other built-in ecosystem still shadows it).
   ...
   - `git/` — libgit2 (`git2`) operations including partial staging and line-level revert via patch manipulation (`patch.rs`). Stash management (`stash_list`/`save`/`apply`/`pop`/`drop`/`clear`, indexed by `stash@{n}`) treats a stash as the commit it is, so `StashEntry.id` is that commit's oid and the UI previews a stash through the ordinary `commit_diff`/`commit_file_contents` path rather than any stash-specific one. `stash_paths` is the path-scoped stash and is **hand-built on purpose**: libgit2's own pathspec stash (`git_stash_save_with_opts` / `Repository::stash_save_ext`) resets with `git_checkout_tree(FORCE | REMOVE_UNTRACKED)` and **no path filter**, so it reverts the whole working tree and deletes every untracked file while stashing only the pathspec — git2's own test asserts that data loss as if it were correct. So the commit is assembled to the shape `stash.c` produces (index-tree commit parented on HEAD, untracked-tree commit with no parents, working-tree commit parented on all of them **in that order**) and only the named paths are reverted. Three things fail *silently* if changed and each is pinned by a test named after it: **`refs/stash` is not auto-reflogged** — `git_refdb_should_write_reflog` covers only `refs/heads`, `HEAD`, `refs/remotes` and `refs/notes`, so `reference_ensure_log` plus an explicit append is what stops the first stash in a repository being written and then invisible to `stash_list` (which reads the reflog); an **untracked path goes in the untracked tree only**, because adding it to the working tree too makes `stash_pop` check it out twice and leave it staged; and the trees are built on a **detached** `Index::new()` using `Index::add` with a hand-built entry, never `add_path`/`add_frombuffer`, which need an owning repository and fail on a bare index. `attribution.rs` matches recorded agent edits onto the current diff **by content only** (recorded line numbers are deliberately discarded), and `grouping.rs` collapses hunks into intent cards: stated intent, then formatting-only, then enclosing symbol. Governing rule: a wrong label is much worse than no label, so every threshold abstains rather than guesses. **libgit2 ref writes are not safe to run concurrently:** every branch delete rewrites the shared `packed-refs`, so firing N `git_delete_branch` calls at once makes them race — one blocks on `packed-refs.lock` (`code=Locked`) while another reads a half-rewritten `packed-refs` (`could not read (expected N bytes, read N-k)`). Any bulk ref operation must be **serialised** (the History tab's `historyLogic.bulkDeleteBranches` awaits each delete in turn for exactly this reason); a branch git legitimately refuses — not fully merged, or checked out in a linked worktree — is a separate, real failure to report, not the same bug.
   ...
   - `dap/` — the Debug Adapter Protocol layers behind breakpoints and stepping. **The crate holds only the pure layers** — `protocol`, `sequence`, `positions`, `breakpoints`, `registry`, `model`, `coalesce`, all tested with no debugger installed. Transport, session and lifecycle live in `src-tauri/src/commands/debug.rs` (stdio for .NET, loopback TCP for Node), and the UI stops at **launch-attached** debugging: console output, state and Stop. `breakpoints`/`positions`/`sequence` are therefore ready and unused — there is still no breakpoint, stack or stepping surface. Sits beside `inspect/` and is not a version of it: `inspect/` reads an *unsuspended* heap and calls no method, this stops the process and asks the runtime's own debugger. Four things to know before touching it. **A client that stops draining the adapter's stdout deadlocks the debuggee** — the pipe fills, the adapter blocks writing, and since it writes from inside a runtime debug callback that holds every debuggee thread suspended, the application never resumes: it freezes with no output, no error and no exit, looking exactly like one that is merely quiet. So reading is decoupled from emitting (`pump_protocol` → an **unbounded** queue → `run_protocol`); a bounded queue would reintroduce the wait that is the whole bug. Depth is not what bounds the cost — `dap::coalesce` merges the backlog, collapsing hundreds of events into one IPC message, and NetCoreDbg makes that necessary by emitting **two** `output` events per logged line. Two rules there are load-bearing and tested: `stdout` and `stderr` never merge (the console ranks an unmarked line by its stream), and a chunk over `MAX_BATCH_BYTES` is never *split* (a cut could land inside a multi-byte character or mid-line). Batching must also never withhold — the flush happens on the path that is about to block, not after it, or a quiet app's last and most interesting line sits in the buffer. **Framing is not duplicated** — DAP frames exactly as LSP does, so `lsp::framing` is reused, as are `lsp::registry`'s `Probe`, `EDITOR_DIRS`, `CSHARP_EXTENSION_PREFIX` and `parse_extension_version` (all `pub(crate)` for this). **The envelope is not JSON-RPC**: a response points back with `request_seq` — spelled in *snake case*, alone in the base protocol, so `Response` carries no `rename_all` and a test pins it; camel-casing it decodes every response with `request_seq: 0` and the debugger appears to hang on its first request. And the governing rule is the sharpest in the crate: *not running*, *starting*, *running*, *paused*, *exited* and *no adapter installed* are **six** answers (`model::DebugState`), a breakpoint the user set and one the adapter *bound* are different facts with *pending* and *rejected* further split (`breakpoints::BindState`), and nothing corrects an adapter that ignores the negotiated line base — the only evidence is a number consistent with both conventions, so `positions` clamps the impossible and shows the adapter's own reported line rather than guessing. **Both ecosystems are supported and both adapters ship with the installer.** `pnpm debuggers:fetch` vendors NetCoreDbg (stdio) and the standalone js-debug DAP server (loopback TCP) into `src-tauri/resources/debuggers/`, pinned by version and SHA-256; `resolve` takes a `bundled_dir` and searches **pin → bundle → PATH**, an absent or empty resource directory being an ordinary answer. The vsdbg inside Microsoft's C# extension is deliberately **not** discovered — its runtime licence restricts it to the Visual Studio product family — so do not "fix" a not-found error by re-adding that probe; `CB_DAP_DOTNET` is the escape hatch. A **`.js`** adapter with no `node` on PATH is refused at resolution rather than at spawn, because the spawn layer's "program not found: node" cannot say why this app wanted Node.
   ...
   - `files.rs` — workspace file access for the Run tab's tree and editor: `list_dir` (lazy, one directory per call, filtered like the project scan), `read_file`/`write_file`, and the mutations behind the tree's right-click menu — `create_file`, `create_dir`, `rename`, `delete`. Every one goes through the private `resolve`/`resolve_new`, which refuse an absolute path, a drive prefix, `..`, the empty path (it names the workspace root), and — for a create — a path that already exists, because this is reached from a *New file* menu where clobbering is unrecoverable.
...
- The Run controls live in the Project tab's toolbar, which is shown only
  while the active editor tab is a file. Stop is one split button. Its caret lists only
...
- **A bound key never reaches the WebView.** `dispatchShortcut` returns whether a
  binding *matched*, not whether the command did anything, because the caller
  uses that answer to `preventDefault`. The two differ exactly where it hurts:
  F5 is bound to Run, and while the Run button was disabled the key fell through
  and **reloaded the whole application**. Run is `allowInText` for the same
  reason — F5 is a function key, so it must fire with the caret in an editor.
- **F5 does whatever Run means where you are standing.** `SqlView` registers its
  own `run.run` handler that runs the query only while focus is inside the SQL
  console and returns `false` otherwise, handing the key back to the Run
  toolbar — the fall-through `FileTree` already uses for `tree.reveal`. Binding
  F5 to `sql.run` instead would not work: `dispatchShortcut` takes the *first*
  command whose chord matches, so two commands sharing a key means one never
  fires.
- Keyboard commands resolve through `shortcutLogic.pickCommandTarget`: several
  codebases are mounted at once and a background one is only `hidden`, so the
  DOM fallback acts on the first **rendered** `[data-command]` match, never the
  first match. A match that exists but is off screen is refused rather than
  used — acting on a codebase the user cannot see is worse than doing nothing.
  Visibility is `getClientRects().length > 0`, not `offsetParent`, which is null
  for the `position: fixed` floating panels.
- Optional features have their own titlebar **Plugins** menu
  (`components/pluginMenuLogic.ts`), not a row in the terminal split button —
  that menu is about *running things*, and a database console is not one. Adding
  a plugin is a row in that module's `PLUGINS` table and nothing else. A
  switched-off plugin is **omitted**, not disabled ("you turned this off" needs
  no arguing with), while one that needs a codebase and has none is a **disabled
  row with a reason** (a state the user is one click from leaving). The button
  itself is hidden when no plugin would appear, and `features === null` means
...
- The app has one global **notification service** (`components/notificationLogic.ts`
  + `NotificationHost.tsx`), rendered by `App`. It exists for what the workspace
  tab signals cannot say: a launcher entry's `cwd` may sit outside every open
  codebase, so a service dying is often nobody's tab to outline. A **persistent**
  launchable ending raises one — including a clean `exit 0`, because `persistent`
  is a statement that the process was meant to stay up, so an orderly exit is
  still unexpected *by the user's account of it*. A `cancelled` end never
  notifies: telling someone the thing they just stopped has stopped is the noise
  that teaches people to ignore the surface. Errors never auto-dismiss (the case
  the service exists for is something ending while the user looked elsewhere);
  quieter kinds do. Notifications about one process dedupe on its key, so a
  crash-looping service is one updating entry rather than a toast per restart.
- A launched command is attributed to the workspace containing **its own `cwd`**
  (`launcherLogic.isInside`, mirroring `recents::within_root`), never to
  whichever codebase is in front. A `global`-group entry belongs to no open
  workspace, and blaming the foreground one outlined an unrelated tab red.
- A saved launchable may be a **command shortcut** (`shortcut`), a long-running
  service (`persistent`, never auto-restarted), and/or `headless` (no output tab
  is minted, though the process still registers in Running and a failure still
  surfaces). Shortcuts share the pinned entries' exemption from the 30-entry
  recents cap — a cap that could evict one would make saving it a suggestion
  rather than a promise.
- Debug is a separate action beside Run, and every decision about it lives in
  `src/views/debugLogic.ts`. `debugAvailability` disables the button exactly
  where `start_debug` would refuse and states *why* in the tooltip — including
  validating a **compound's members**, since the backend preflights all of them
  and one member with no adapter must not build the others first.
  `debugEffects` maps a `DebugEvent` onto the console's `ProcessEvent` plus a
  tab status, and is the one place the six `DebugState` answers could collapse
  into each other: a missing adapter keeps everything it looked for, a failure
  keeps its detail, `notRunning` produces nothing, and a **null exit code is
  not a failure** (it is what a Stop or a replacement launch reports).
