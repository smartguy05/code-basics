...
Renaming a Rust field fails one of these tests, and the failure message points at the TypeScript file that needs the matching edit. Crossing types defined outside `model.rs` pin their keys beside their own module instead — `workspace_serialises_with_the_keys_the_ui_reads` in `workspace.rs`, `dir_entry_serialises_with_the_keys_the_ui_reads` in `files.rs`, the terminal's `TerminalEvent` in `pty/model.rs` (each variant's keys pinned, and a signalled exit kept as an explicit `null` code — the same "no `skip_serializing_if`" rule as `ProcessEvent`), the notes `Note`/`NotesFile` in `notes_tests.rs` (`serialisation_shape_pins_the_wire_keys`), the launcher's `Launchable`/`LauncherFile`/`LauncherGroups` in `launcher/model_tests.rs` (where `label` is pinned as an explicit `null` rather than an omitted key, so "never renamed" is visible on the wire), and the inspector's types in `inspect/model_tests.rs`:
...
Six more cross for rename (`RenameResult`, `RangeEdit`, `RenamedFile`, `BufferEdits`, `RenameFailure`, `PrepareRenameResult`), pinned in the same file:

- `rename_result_serialises_with_the_keys_the_ui_reads`, `prepare_rename_result_serialises_with_the_keys_the_ui_reads`, `a_rename_failure_says_whether_the_file_is_beyond_recovery`, `every_rename_type_round_trips_through_json`.
- `a_rename_that_could_not_be_asked_carries_the_total_key_as_null` — the same `Some(0)` / `None` split as `UsageResult.total`, and here it separates "the server found nothing to change" from "nobody was asked".
- `a_symbol_that_cannot_be_renamed_is_ready_and_says_so_rather_than_failing` and `a_prepare_rename_that_could_not_be_asked_is_not_renameable_and_carries_no_range` — `prepareRename` answering `null` is a true answer *about the caret*, so the outcome stays `"ready"` with `renameable: false`; reporting `"failed"` would blame a server that is working perfectly. `PrepareRenameResult::not_renameable` is a constructor of its own for exactly that reason.
- `RangeEdit` is the only type on this surface with an **end** position, and both ends follow the same rule: 1-based `line`, 0-based UTF-16 `character`, half-open (`end` exclusive). It carries **no byte offsets** — the backend has no text for an open buffer, so the conversion is the editor's. A zero-width range is an insertion and the *normal* case rather than an edge one: the real Roslyn server answers a rename with a minimal diff, so `Walker` → `HeapWalker` arrives as an insertion of `"Heap"`.
- `RenameResult` carries two lists of edits on purpose. `written` is what the backend already wrote to disk (closed files only); `buffers` is for the editor to dispatch, because a disk write behind an open tab is invisible to its CodeMirror buffer and clobbered by the next `Mod-s`. The frontend must dispatch `buffers` synchronously and **report** an entry with no receiving editor rather than dropping it.
- `RenameFailure.unrecoverable` is a plain always-serialised `bool` for the `AttachableProcess.isApplication` reason: it is the one state nothing can put right — a write failed *and* an earlier file's restore failed too — and the UI has to escalate on it without parsing English.
