...
- **Rust:** `crates/core/src/model.rs` (plus `workspace.rs`, `git/repo.rs`, `git/patch.rs`, `process/mod.rs`, and the small response structs in `src-tauri/src/commands/`), all deriving `Serialize`/`Deserialize` with `#[serde(rename_all = "camelCase")]`.
...
Renaming a Rust field fails one of these tests, and the failure message points at the TypeScript file that needs the matching edit.
