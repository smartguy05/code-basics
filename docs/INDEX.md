...
| `crates/core/src/adapters/dotnet_tests.rs` | 1331 | Tests for the .NET adapter. |
| `crates/core/src/adapters/manifest.rs` | 519 | Declarative adapters: adding an ecosystem without writing Rust. |
...
| `crates/core/src/changelists_tests.rs` | 226 | Tests for change groups. Included by `changelists.rs` under `#[cfg(test)]`. |
| `crates/core/src/config.rs` | 630 | The workspace configuration file, `.code-basics/config.json`. |
...
| `crates/core/src/git/attribution_tests.rs` | 955 | Tests for attributing diff lines to recorded edits. |
...
| `crates/core/src/git/repo.rs` | 1386 | Repository reads and mutations. |
...
| `crates/core/src/inspect/session.rs` | 953 | The decisions that surround one capture. |
| `crates/core/src/inspect/session_tests.rs` | 1307 |  |
| `crates/core/src/inspect/sidecar.rs` | 385 | Deciding how to call the inspector, without calling it. |
| `crates/core/src/inspect/sidecar_tests.rs` | 381 |  |
...
| `crates/core/src/intents/hook.rs` | 359 | Turning a hook payload into a record. |
| `crates/core/src/intents/hook_tests.rs` | 404 | Tests for ingesting hook payloads. Included by `hook.rs` under `#[cfg(test)]`. |
| `crates/core/src/intents/intents_tests.rs` | 305 | Tests for recorded agent intent. Included by `mod.rs` under `#[cfg(test)]`. |
| `crates/core/src/intents/mod.rs` | 373 | What a coding agent said it was doing, and where it wrote it down. |
...
| `crates/core/src/intents/providers/claude_code.rs` | 452 | Claude Code: hooks in `settings.json`, history in per-project transcripts. |
| `crates/core/src/intents/providers/codex.rs` | 494 | Codex: hooks in `hooks.json`, history in dated rollout files. |
...
| `crates/core/src/intents/providers/providers_tests.rs` | 508 | Tests for provider detection and hook installation. |
| `crates/core/src/lib.rs` | 23 | Core logic for `code-basics`. |
| `crates/core/src/model.rs` | 468 | Types shared between the Rust core and the TypeScript frontend. |
...
| `crates/core/src/process/mod.rs` | 562 | Process supervision: spawn, stream, cancel. |
| `crates/core/src/secrets.rs` | 433 | .NET user secrets: per-project secrets stored *outside* the repository. |
...
| `crates/core/src/workspace.rs` | 1037 | Scanning a workspace for projects and building the configurations that can |
...
| `src/components/ObjectTree.tsx` | 406 |  |
...
| `src/views/InspectView.tsx` | 1262 |  |
| `src/views/RunView.tsx` | 1078 |  |
| `src/views/TestsView.tsx` | 485 |  |
...
| `src-tauri/src/commands/git.rs` | 295 | Git commands. |
...
| `src-tauri/src/commands/run.rs` | 313 | Running applications and tests. |
...
| `src-tauri/src/invocation.rs` | 187 | Turning a run configuration into a command line. |
| `src-tauri/src/lib.rs` | 135 | The Tauri shell. |
...
- `crates/core/src/inspect/session.rs`: `new_session_id()`, `request_for()`, `attribute()`, `launcher_caveat()`, `live_target_reason()`, `dump_for_run()`, `unsupported_reason()`, `attach_caveats()`, `first_bitness()`, `missing_sidecar_reason()`, `status()`, `arm_dumps()`, `ArmedDumps`, `prune()`
