...
A few things are shared across every open codebase rather than kept per-codebase: the [Notes](#notes) scratchpad (one global panel), [appearance and app shortcuts](customizing.md), and the global chrome that follows whichever codebase is in front — the branch widget and the bottom status bar. The run-configuration dropdown, by contrast, lives in each codebase's own Run toolbar.
