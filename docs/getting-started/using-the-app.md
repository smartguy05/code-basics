...
Appearance, font sizes, and app-owned keyboard shortcuts are global rather than
workspace settings. See [Customizing code-basics](customizing.md).

The titlebar starts with a menu bar: a **File** menu (Open, Rescan, Exit — the standalone Open…/Rescan buttons on the right remain too), an **Enhancements** menu with **Instructions** and **Prompts** submenus (see [Enhancements](../guides/instruction-enhancements.md)), and a **Help** menu holding **About code-basics…**. Opening a folder from here closes any open editor tabs.

**About** reports the build you are actually running: the app version, the commit it was built from, the build date, the Tauri version, and the OS and architecture. The commit is marked `-dirty` when it was built from a modified tree, because an unmarked sha would be a *wrong* statement about which code is running — and where any of it could not be established (a source tarball with no `.git`, or no `git` on the build machine) the field reads `unknown` rather than going blank. The commit and the build date are shown as a pair on purpose: an unstaged edit made after a build does not re-stamp the binary, so the date is what tells you how old the reading is.

When the workspace is a git repository, the **centre** of the titlebar shows a branch widget (⎇ branch, with ahead/behind counts): a Rider-style menu with fetch/pull/push, a new-branch field, and click-to-switch (× deletes a branch). Like the other titlebar menus it draws **over** the floating panels — a terminal or the Notes panel used to cover it, and cover the backdrop that closes it, so clicking the terminal did neither. The menu opens with a **filter box focused**, so you can type part of a branch name — it matches the *full* name, so `feat` finds `origin/feature/x` and `origin` finds every remote — and the folders on the way to a match open themselves rather than hiding it. It is available from every tab; the History tab keeps the full console output for network operations. The five view tabs are on their own row below the titlebar, and a **status bar along the bottom** shows the active workspace's folder name and full path, with the **language-server status right-aligned** in it (silent unless a server is starting, missing or dead — hover it for what to install).

On the right of the titlebar are, in order:

- **Plugins ▾** — the optional features. The **SQL console** lives here: it opens as a floating window rather than a tab, one per codebase, dragged and minimized like the Notes panel and kept mounted while minimized so a streaming query and its connections survive. **Ask the codebase** is here too — the same box **Ctrl+/** opens, which remains the quicker route; it had a keyboard shortcut and no menu entry at all, so the one optional feature nothing else advertised was also the hardest to discover. The button is hidden entirely when every plugin is switched off, and a plugin that needs a codebase says so on a disabled row rather than doing nothing. Future optional features appear in this menu too.
- **Notes** — the global scratchpad ([below](#notes)).
- The **terminal split button** — its body opens a new terminal, its caret a menu holding **Launch…**, **Running**, **App output** and your [saved command shortcuts](#running-other-apps). It replaced the four separate Launch / Apps / Running / + Terminal buttons, and the counts those carried are now badges on the matching rows.
- **Open…** / **Rescan**.

The run configuration is picked from a dropdown in the Project tab's Run toolbar (see [Project](#project)), not the titlebar.

**F5 runs.** With focus in the SQL console it runs the query; anywhere else it runs the selected run configuration. It is a function key rather than a character, so it fires even while the caret is in the editor or the query box. Settings' keyboard page groups the commands into **Application** and one section per **plugin** (the SQL console's own commands are in its own block), so rebinding a plugin's keys does not mean hunting through the app's.
...
You can open more than one codebase at once. Each opens as a tab in the strip above the view tabs: click a tab to bring that codebase to the front, **×** to close it, and **+** (or **Open…**) to add another. Opening a folder that is already open simply brings it forward and rescans it. When two open codebases share a folder name, each colliding tab is prefixed with the parent directory that tells them apart (`one/api`, `two/api`). The active codebase's name and full path show in the status bar along the bottom.

**Rename a tab** by double-clicking it, or right-click it for **Rename…** (Enter or clicking away commits, Esc abandons). **Reset name** in the same menu drops the custom label and follows the scan again. Names are capped at 40 characters and are yours alone — they are kept in `localStorage` against the codebase's path, never written into the repository, so a rescan or a re-open does not lose them.

Every codebase is fully independent — its own Project/Tests/History/Architecture/Objects, its own run configurations, terminals, and language server. A codebase you switch away from is **hidden, not closed**: its running apps and tests keep running, its terminals keep streaming, and its language server stays loaded, so switching back is instant and nothing is interrupted. A background codebase tells you what happened in it by **outlining and pulsing its tab**: failures from builds, tests, terminals, or launched apps remain red until viewed; a hidden terminal bell remains amber until acknowledged; and successful background completion flashes green briefly before expiring. Stopping a task yourself is quiet. When two things happen at once the louder one wins — a later success never turns a failed project's tab green. The tab you are already looking at never flashes.

A few things are shared across every open codebase rather than kept per-codebase: the [Notes](#notes) scratchpad (one global panel), [appearance and app shortcuts](customizing.md), the [notifications](#notifications), and the global chrome that follows whichever codebase is in front — the branch widget and the bottom status bar. The run-configuration dropdown, by contrast, lives in each codebase's own Run toolbar.
...
The tabs are **Project · Tests · History · Architecture · Objects**. Project is first and opens by default.

### Project

One tab for reading code and for reviewing what you have changed. An **icon rail** down the side of the panel switches it between two surfaces over **one shared editor area**:

- **Files** — the directory tree of the workspace.
- **Changes** — the working-tree list, the intent cards, erosion, stashes and the commit box.

Switching the rail is navigation, not a tab change: your open files, the running processes and the console all stay exactly as they were, and only the side panel changes.

**A changed file's diff opens as another editor tab**, beside the files you have open, rather than replacing the main area — so a diff and the source you were reading sit side by side in the same strip. A diff tab is identified by its file *and* its comparison mode, so the same file compared against the index and against HEAD are two tabs, each labelled with its mode. Diff tabs are read-only and are deliberately kept out of the back/forward navigation stack.

**The toolbar follows the active editor tab, not the rail.** A file tab shows the Run toolbar (run, debug, build, stop, env…); a diff tab shows the Changes toolbar (comparison mode, staging, revert). That way the buttons above the editor always act on what is on screen — the alternative would arm Stage and Revert over a source file you were merely reading.

The two halves are documented below: [running and editing](#running-and-editing) and [reviewing changes](#reviewing-changes).
...
The Project tab's **Files** rail, and the Run toolbar above the editor.

Application launches: .NET executable projects (including `launchSettings.json` profiles) and `package.json` scripts. Rust crates are detected too — a `Cargo.toml` becomes a project, classified as an executable or a library — but detection is all it is: no `cargo run` or `cargo test` configuration is offered, deliberately, since that would add entries to this tab and the Tests tab for every Rust repository. To run Rust, drop in the `cargo-nextest` [declarative adapter](../guides/adding-an-ecosystem.md); it supplies configurations for the same directory rather than being shadowed by the built-in detection. Configurations are picked from the dropdown in the Run toolbar, beside the **Env** picker — its status dot is grey when idle, yellow while building/starting, green once the app is up, red on failure. **Run** and **Stop** are icon buttons with tooltips (the tooltip states the key currently bound, so it cannot drift from what the key does — **F5** runs by default). **Run** starts normally; **Debug** launches .NET or Node through a real debug adapter, so debugger-dependent application behavior is active. Debug output and lifecycle share the same console and Stop controls. Both debug adapters — NetCoreDbg for .NET and the standalone js-debug server for Node — **ship with the installer**, so a fresh install can debug with nothing else to download. (Node debugging still needs `node` itself on `PATH`, which anyone debugging a Node app already has.) `CB_DAP_DOTNET` and `CB_DAP_NODE` override the bundled copies, and a copy on `PATH` is used when neither is available. A missing adapter is explained, and the application is never launched without one — a Debug button that quietly does not debug is worse than one that says why. Output streams to a console as it is produced, including bare-`\r` progress redraws. Each run (and each build action) gets its own console tab labeled with the configuration name, so running several projects at once keeps their output separate; closing a tab does not stop its process.

**Build configuration is a separate picker**, not a separate configuration. A .NET project appears in the list once per target framework; Debug, Release and anything else in `<Configurations>` are chosen from the dropdown beside **Env**, and the choice is remembered per configuration (locally — it is a personal preference, not something the team inherits from `config.json`). It applies to **Build** and **Rebuild** as well as to running, so the binaries you build are the ones the next run starts. Ecosystems that have no such concept — Node, cargo, declarative adapters — do not get the picker at all rather than an invented Debug/Release pair.

**Stop** is a split button rendered as one control: **Stop | ▾**. Clicking its main half stops the selected configuration; the caret lists only configurations launched by the Run button, across every open codebase and with this codebase first. Terminals, builds, launched commands, review panels and before/after runs do not appear here because they have their own lifecycle controls. A Run process the app can no longer address normally is listed last under **Left over**, and killing it asks first (its identity is re-checked before anything is killed, so a recycled pid is refused). **Stop all applications** at the foot of the menu stops the Run applications in this codebase.

The **Files** rail is a directory tree of the workspace, filtered like the project scan (no `node_modules`, `bin`, `obj`, …) but with no depth limit — each directory is listed the first time it is expanded. Every row carries a **file-type icon** (the Changes list uses the same ones). Clicking a file opens it in the editor area above the console (syntax highlighting per extension, tabs per open file). A small bar above the tree holds two glyph buttons: **Select opened file** (**Alt+F1**) expands the tree down to whatever the editor is showing and scrolls to it — disabled, with the reason in its tooltip, when nothing is open — and **Collapse all** closes every folder. **Right-click** a file or folder — or the empty space below the tree, which targets the workspace root — for **New file…**, **New folder…**, **Rename…** and **Delete…**. A name may contain folders (`views/parts/Panel.tsx` creates the folders on the way), a new file opens in the editor immediately, and a rename with slashes in it moves the file, which is the only move gesture the tree has. Deleting asks first and is permanent — it does not go to the recycle bin — and it closes the editor tabs for that path and anything beneath it, because the editor saves on a timer and a tab left pointing at a deleted file would recreate it. **Ctrl+S** saves, **Ctrl+/** toggles line comments for the file's language, and **Ctrl+G** jumps to a line (as in Rider — **Ctrl+F** finds within the file); unsaved files show a ● on their tab, and closing such a tab discards the changes. The divider between editor and console drags to resize, and the split persists.

The file tabs behave like a browser's. **The back and forward mouse buttons step through the files you have been looking at** — open one file, then another, and *back* returns you to the first; this includes jumps made by [middle-clicking a symbol](#finding-where-a-method-is-used) to go to its definition, so *back* brings you home from a jump into another file. This works while the Project tab is on screen. **Pin a tab** with the 📌 that appears on it (hover a tab, or it stays lit once pinned): pinned tabs move to a separate row above the rest and stay put, so a file you keep returning to is not lost among the others. Middle-click still closes a tab, pinned or not.
...
The dropdown's list can be arranged to taste: the ☆ on each row stars it as a favourite (favourites sort first), ↑/↓ move a row within its group, and ✎ opens that row's configuration in the editor. Favourites and ordering are saved to `config.json`. **+ New configuration…** and **Import from Rider…** live at the bottom of the same menu. The pencil is per row rather than a toolbar button beside the dropdown, which is where it used to be: that button could only ever edit the *selected* configuration, so editing a different one meant selecting it first — and selecting it also changes what F5 runs.
...
For .NET configurations, **Secrets…** opens a project's [user secrets](../reference/configuration.md#net-user-secrets) as an ordinary editor tab (`secrets.json`, with JSON highlighting and Ctrl+S to save) — the Rider way — rather than a modal. When the workspace has **more than one** .NET project, **Secrets…** is a dropdown so you can pick which project's secrets to open (every .NET project in the workspace, not only the selected configuration's); with a single .NET project it opens directly. It is the same store `dotnet user-secrets` and Rider manage, kept under your user profile rather than in the repository. Discovery evaluates MSBuild when the id is inherited from `Directory.Build.props`, an import, or a conditional property instead of appearing directly in the project file. Saving for a project without a direct `<UserSecretsId>` adds one to the `.csproj` first; opening the tab changes nothing until you save.

### Reviewing changes

The Project tab's **Changes** rail, and the Changes toolbar that appears above a diff tab. Clicking a changed file opens its diff as an editor tab beside your open files; the rail stays on Changes, so you can work down the list without losing your place. Working-copy review:
...
- Stage/unstage whole files, hunks, or individually selected lines. **Right-click a file** to stage or unstage it without opening it, or to **stash it** — see below.
- The list has four views, chosen with the **Files / Intent / Erosion / Stashes** toggle at the top:
  - **Files** — the file list grouped into **Staged**, your own named **change groups**, and **Unstaged**. Within each section the files are drawn as a **collapsible folder tree** by default; the **Layout** control above the list switches to a flat list of full paths, and whichever you choose is remembered, as is which folders you folded away (per repository — the folds are paths, and they mean nothing in another codebase). Groups are for organising work in progress — right-click a file to move it into one, or use "+ New group". They are local to you and never committed; see [change groups](../reference/configuration.md#change-groups-changelistsjson). A partially staged file shows under both Staged and its unstaged group, as `git status` reports it. **Ctrl-click** (or Cmd-click) picks out several files and **Shift-click** takes a run of them; right-clicking inside that selection acts on all of it, and right-clicking outside it acts on the one row instead. Selecting files this way is how you **stash just part of your work** — see Stashes below.
  - **Intent** — the same changes grouped by the decision behind each, as cards you can stage or revert as a unit. Because this view has no Staged section of its own, each card and file carries a **staged** / **partial** badge so you can see what is already in the index without switching back to Files. See [agent intent capture](../guides/agent-intent-capture.md). When the grouping gets it wrong, **right-click a card to move all of it, or one of its file rows to move just that file**, into any other card or a new one you name — see [grouping the cards yourself](../guides/agent-intent-capture.md#grouping-the-cards-yourself). Reasons are **retired once your commits absorb them**, so an intent from work you finished last week stops labelling what you are doing today; retired records are archived rather than deleted, and **Archive absorbed intents…** in the capture setup pane clears a backlog recorded before this existed. The **Run before/after** button here adds *behavioral* evidence: it builds the change against both `HEAD` (in an isolated git worktree, so your working tree is never disturbed) and your working tree, runs the same tests and captures the same console output on each side, and — when a `.http` scenario and a server launch configuration are present — replays those requests against both. It then diffs the observable outcomes (test pass/fail transitions, console differences with noise like timestamps masked out, HTTP status and body changes) and attaches each difference to the card that plausibly caused it, with anything it cannot confidently attribute shown in a separate panel rather than guessed onto a card. The panel keeps the live run under a **Console** tab and the findings under **Evidence**, and switches to Evidence when the run finishes. Every difference expands to the evidence behind it — the console lines that actually appeared and disappeared, an HTTP response's status, header and body changes, which test cases moved and how — with a note when timestamps and ids were masked before comparing, and, for anything unattributed, the reason it pinned to no card. A test case is not mapped to a source file and an `.http` request's handler is not derivable, so those never attach to a card; that is a deliberate abstain, not a failure to find the cause.
  - **Stashes** — every stash, newest first, each showing the branch it was taken on and a read-only preview of what it holds (a stash is a commit, so it opens in the same diff viewer). **+ Stash changes** sets the working tree aside under a message; select a stash to preview it, then **Apply** (keep it in the list), **Pop** (apply and remove), **Drop** (remove one), or **Clear all**.
    ...
    To set aside **only some** of your work, go to **Files**, select the files you want (Ctrl-click and Shift-click both work), right-click and choose **Stash file…** / **Stash N files…**. You are asked for a message, and only those files are stashed and returned to their committed state — every other change stays exactly where it was, staged or not. New files you have never committed can be stashed too: they disappear from the working tree and come back untracked when you pop. A file with unresolved merge conflicts is left out, because there is no single version of it to set aside.
...
touched and opens the first one in the same diff viewer the Project tab uses —
...
The canvas pans by dragging and zooms with the wheel or the **+ / −** buttons; **Fit** frames the whole diagram and **Reset** returns to 1:1. Where you left each diagram is remembered, so glancing at another tab does not cost you the zoom. **Clicking a box opens its file** in the Project tab's editor — for a built-in map that is exact, and for a saved diagram it opens only when the node name matches one indexed symbol unambiguously; otherwise nothing happens rather than the wrong file.
...
Not one of the views but available over all of them: the **body of the titlebar's terminal split button** — its caret opens the menu described above — opens a floating, interactive terminal window. It runs a shell as a real pseudo-terminal, so it is a genuine interactive session — you can launch Claude Code, run a build, tail a log, anything — and type into it, arrow keys and prompts and all. It starts in the open workspace's directory.

**Which shell** is yours to choose. With no preference set it uses the platform default (the first of `pwsh`, `powershell`, `cmd` that is installed on Windows; `$SHELL` on macOS/Linux). **File → Settings → Terminal** lists the shells actually found on this machine and lets you pick the one new terminals use — and the caret menu beside the terminal button has a **New terminal in** section for opening a one-off session in a different shell without changing that default. Both lists show each shell's resolved path, which is what tells two `bash` installations apart.

Only shells that were actually found are offered. One that cannot be located is left out rather than listed broken or greyed out, so an empty list is a real answer and not a failure — terminals still open on the platform default. `wsl.exe` is deliberately never listed: it ships on every modern Windows install whether or not a distribution exists, so its presence is no evidence that a shell would start. If a shell you picked later disappears from the machine, your choice is **kept** (a broken `PATH` or a mid-upgrade tool is usually temporary), the terminal opens on the platform default, and Settings says so.

Each terminal floats over the app like the agent panel: drag it by its header, resize it from the corner, or **minimize** it to a small pill. Minimized terminals keep running, and the pill **flashes** when a hidden session rings the terminal **bell** (`\x07`) — the signal a program uses to ask for you, so Claude Code waiting on input flashes but ordinary streaming output does not. When that terminal belongs to a codebase other than the one on screen, its **workspace tab** pulses amber too, so you can tell which project wants you — and a minimized terminal that simply *finishes* pulses that tab green twice, which is enough to notice and not enough to nag. Open as many as you like — they cascade so they do not stack exactly — and they survive switching between the tabs. **Clicking a terminal brings it to the front**, anywhere on it rather than only its header, so overlapping sessions work the way windows do; a newly opened terminal and one you restore from its pill both come to the front too. The **Notes** panel deliberately stays above them all, since it is the thing you keep beside what you are doing rather than another window competing for the top. **Rename a terminal** by double-clicking its title in the header (Enter or clicking away commits, Esc abandons), or right-click the header for **Rename…**. The name shows in the header, on the minimized pill, and on the terminal's row in the Running panel. Like codebase-tab names it is capped at 40 characters and stripped of control characters; a name that would be blank is refused and the old one kept. Names are not remembered across a restart, deliberately — terminals do not survive one, and numbering restarts at 1, so a remembered name would end up on an unrelated terminal.

**Copy and paste** use the usual terminal chords: `Ctrl+Shift+C` (or `Ctrl+Insert`) copies the selection, and `Ctrl+V` / `Ctrl+Shift+V` / `Shift+Insert` paste — `Ctrl+C` stays the shell interrupt. Closing a terminal ends its whole process tree, so a shell that launched `claude` (and its `node` child) is cleaned up with it.

## Running other apps

Sometimes what you need is not a project configuration but just another program: a local Redis, `docker compose up`,
a Python script, ngrok. **Launch…**, in the titlebar terminal button's menu, opens a small overlay with one box: type a
command line, press **Enter**, and it runs.
...
pinned ones never do. Three more toggles decide how an entry behaves:

- **⚡ shortcut** — publish it as a named row in the titlebar terminal menu, so it is one click from anywhere. This is not the same fact as a pin (which only sorts the picker), but like a pin it exempts the entry from the recents cap.
- **∞ persistent** — say this is a long-running service. The menu then offers **Stop** while it is up rather than Run, since a second copy of a service is nearly always a mistake, and its stopping is [notified](#notifications). It is never restarted for you.
- **◌ headless** — run it with no output tab. It is still listed in Running and still stoppable.

The list is stored **globally** — `%APPDATA%\code-basics\launchers.json` on Windows
...
floating panel with a tab per app. Each tab has the same console as the Project tab (Ctrl+F search, copy-all, copy
...
Everything launched this way also appears in the **Running** panel — from the titlebar terminal button's menu, as is the **App output** panel — with its pid and age,
...
Some things need saying that belong to no codebase. A launcher command's working directory may sit outside every open project, so there is no workspace tab to outline — the app therefore has a **global notification service**, a small stack of messages in the corner.

What it reports today is a **persistent** service stopping when you did not stop it. That includes a *clean* exit: `persistent` is your statement that this is meant to stay up, so exiting with code 0 on its own is still unexpected, and the message says so in as many words rather than reading like good news. Stopping it yourself is silent — telling you that the thing you just stopped has stopped is the fastest way to make notifications not worth reading — and a failure to start is reported with the reason.

An **error never dismisses itself**, because the case this exists for is a service dying while you were looking at something else; warnings and informational notes do go away on their own. At most four are kept on screen, oldest dropped first, and repeated news about the same service **replaces** its earlier message rather than stacking, so a crash-looping process leaves one entry that keeps updating.

## Notes

Also available over all views: the **Notes** button in the titlebar opens a floating scratchpad for free-form notes, reminders, and prompts you want to keep for later. Like a terminal it floats over the app — drag it by its header, resize it from the corner, and **minimize** it to a thin labeled bar that expands back when you click it. Clicking **Notes** in the titlebar again also restores the minimized panel.

One panel holds several **named notes**: click **+** to start one, click a tab to switch, double-click a tab to rename it, and the **✕** on a tab deletes it. The color picker belongs to the active note; its color follows that tab and the minimized bar while it is active, and is stored with the note rather than as a color for the whole Notes window. Typing autosaves after a short pause (and again on close), written atomically, so your notes survive an app crash or restart without an explicit save. Notes are stored **globally** — under your user config directory (`%APPDATA%\code-basics\notes.json` on Windows; `$XDG_CONFIG_HOME`/`~/.config` elsewhere), *not* in the workspace — so the same scratchpad follows you into every project. (Set `CB_NOTES_PATH` to override the file location.)
