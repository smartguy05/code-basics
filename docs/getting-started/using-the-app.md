...
**Stop** is a split button. Clicking it stops the selected configuration, as before; the **▾** beside it lists everything the app currently has running — configuration runs, launched apps, builds, terminals, review and before/after panels — grouped by kind with this codebase first, so a process you started from another tab and forgot is one click away. Anything the app started but can no longer address normally is listed last under **Left over**, and killing one of those asks first (its identity is re-checked before anything is killed, so a recycled pid is refused). **Stop all applications** at the foot of the menu is the old *Stop All*: it stops running applications in this codebase and deliberately leaves tests, builds, terminals and launched apps alone — stop those individually above.
