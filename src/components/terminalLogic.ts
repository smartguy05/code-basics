...
// Terminal stacking is now one slice of the app-wide floating-panel focus order
// (`focusOrderLogic`), which spans terminals, the browser and Notes so "last
// clicked is on top" is a single fact. These re-exports keep the old
// terminal-named API — and its tests — unchanged; the implementations live in
// `focusOrderLogic` under the general names.
export {
  raiseFocus as raiseTerminal,
  syncFocusOrder as syncStackOrder,
  focusOffset as stackOffset,
  FOCUS_STACK_SPAN as TERMINAL_STACK_SPAN,
