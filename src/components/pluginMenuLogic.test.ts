...
const ON = [feature("sqlConsole", true)];
const OFF = [feature("sqlConsole", false)];

describe("pluginMenuRows", () => {
  it("offers the SQL console when its feature is on and a codebase is open", () => {
    const rows = pluginMenuRows({ features: ON, workspaceOpen: true });
  ...
  it("uses the same name Settings uses", () => {
    ...
    expect(pluginMenuRows({ features: ON, workspaceOpen: true })[0]?.label).toBe("SQL Console");
  });
  ...
  it("disables — and explains — a plugin that needs a codebase when none is open", () => {
    ...
    const rows = pluginMenuRows({ features: ON, workspaceOpen: false });
